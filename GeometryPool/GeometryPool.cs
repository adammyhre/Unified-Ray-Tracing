...
namespace UnifiedRayTracing.Geometry {
    ...
    public class GeometryPool : IDisposable {
        const int kMaxThreadGroupsPerDispatch = 65535; // Counted in groups, not threads.
        const int kThreadGroupSize = 256;              // Counted in threads

        static class GeoPoolShaderIDs {
        ...
        static int DivUp(int x, int y) => (x + y - 1) / y;

        const GraphicsBuffer.Target VertexBufferTarget = GraphicsBuffer.Target.Structured;
        const GraphicsBuffer.Target IndexBufferTarget = GraphicsBuffer.Target.Structured;
        ...
        void LoadKernels(ComputeShader geometryPoolShader) {
        ...
        int CalcVertexCount(int bufferByteSize) => DivUp(bufferByteSize, GetVertexByteSize());
        int CalcIndexCount(int bufferByteSize) => DivUp(bufferByteSize, GetIndexByteSize());
        int CalcMeshChunkTablesCount(int bufferByteSize) => DivUp(bufferByteSize, GetMeshChunkTableEntryByteSize());

        void DeallocateGeometrySlot(ref GeometrySlot slot) {
        ...
        void DeallocateGeometrySlot(int geoSlotHandle) {
            var geoSlot = m_GeoSlots[geoSlotHandle];
            Assert.IsTrue(geoSlot.valid);
            --geoSlot.refCount;
        ...
        bool AllocateGeo(Mesh mesh, out int allocationHandle) {
            ...
            if (m_MeshHashToGeoSlot.TryGetValue(meshHash, out allocationHandle)) {
                ...
                Assert.IsTrue(geoSlot.hash == meshHash);
                Assert.IsTrue(geoSlot.meshChunkTableAlloc.block.count == mesh.subMeshCount);
            ...
            if (mesh.subMeshCount > 0) {
                ...
                if (!newSlot.meshChunkTableAlloc.valid) {
                    newSlot.meshChunkTableAlloc = m_MeshChunkTableAllocator.GrowAndAllocate(mesh.subMeshCount, (int)(GraphicsHelpers.MaxGraphicsBufferSizeInBytes / GetMeshChunkTableEntryByteSize()),
                        out int oldCapacity, out int newCapacity);

                    if (!newSlot.meshChunkTableAlloc.valid)
                        throw new UnifiedRayTracingException($"Can't allocate a GraphicsBuffer bigger than {GraphicsHelpers.MaxGraphicsBufferSizeInGigaBytes:F1}GB",
                ...
                for (int submeshIndex = 0; submeshIndex < mesh.subMeshCount; ++submeshIndex) {
                    ...
                    if (!newMeshChunk.vertexAlloc.valid) {
                        newMeshChunk.vertexAlloc = m_VertexAllocator.GrowAndAllocate(submeshDescriptor.vertexCount, (int)(GraphicsHelpers.MaxGraphicsBufferSizeInBytes / GetVertexByteSize()),
                            out int oldCapacity, out int newCapacity);
                    ...
                    if (!newMeshChunk.indexAlloc.valid) {
                        newMeshChunk.indexAlloc = m_IndexAllocator.GrowAndAllocate(submeshDescriptor.indexCount, (int)(GraphicsHelpers.MaxGraphicsBufferSizeInBytes / sizeof(int)), out int oldCapacity,
                            out int newCapacity);
            ...
            if (m_FreeGeoSlots.IsEmpty) {
                ...
                Assert.IsTrue(!m_GeoSlots[allocationHandle].valid);
        ...
        void DeallocateGeoPoolEntrySlot(GeometryPoolHandle handle) {
        ...
        void DeallocateGeoPoolEntrySlot(ref GeoPoolEntrySlot geoPoolEntrySlot) {
        ...
        void UpdateGeoGpuState(Mesh mesh, GeometryPoolHandle handle) {
            ...
            if (!geoSlot.hasGPUData) {
                ...
                Assert.IsTrue((buffer.target & GraphicsBuffer.Target.Raw) != 0);
        ...
        uint FNVHash(uint prevHash, uint dword) {
        ...
        uint CalculateClusterHash(Mesh mesh, GeometryPoolSubmeshData[] submeshData) {
            uint meshHash = (uint)mesh.GetHashCode();
        ...
        static int FindSubmeshEntryInDesc(int submeshIndex, in GeometryPoolSubmeshData[] submeshData) {
            if (submeshData == null)
                return -1;
        ...
        public bool Register(in GeometryPoolEntryDesc entryDesc, out GeometryPoolHandle outHandle) {
            ...
            if (m_GeoPoolEntryHashToSlot.TryGetValue(geoPoolEntryHash, out outHandle)) {
                ...
                Assert.IsTrue(geoPoolEntrySlot.hash == geoPoolEntryHash);

                GeometrySlot geoSlot = m_GeoSlots[geoPoolEntrySlot.geoSlotHandle];
                Assert.IsTrue(geoSlot.hash == (uint)mesh.GetHashCode());
            ...
            if (m_FreeGeoPoolEntrySlots.IsEmpty) {
                ...
                Assert.IsTrue(!m_GeoPoolEntrySlots[outHandle.index].valid);
        ...
        void LoadVertexAttribInfo(Mesh mesh, VertexAttribute attribute, out VertexBufferAttribInfo output) {
            ...
            Assert.IsTrue((output.buffer.target & GraphicsBuffer.Target.Raw) != 0);
        }
        ...
        void AddIndexUpdateCommand(
            ...
            Assert.IsTrue(indexCount <= location.block.count);
            Assert.IsTrue(outputOffset < location.block.count);
